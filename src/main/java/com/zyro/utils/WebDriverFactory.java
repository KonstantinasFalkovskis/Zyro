...
public class WebDriverFactory implements DriverFactory {
    ...
    public WebDriverFactory(Environment env) {
        ...
        if (env.containsProperty("webDriver.selenium.proxy.host") &&
                env.containsProperty("webDriver.selenium.proxy.port")) {
            proxy = new Proxy();
            String proxyDetails = String.format(
                    "%s:%d", env.getRequiredProperty("webDriver.selenium.proxy.host"),
                    env.getRequiredProperty("webDriver.selenium.proxy.port", Integer.class)
            );
